...
5. **System Design Interview, Vol 1 & 2 (Alex Xu)**
   ...
   - Vol 1, Ch 11: Design a News Feed (real-time fan-out, similar to target employer's data distribution)
   - Vol 2: specific designs matching target employer's domain -- real-time data processing, geospatial systems, notification/alerting pipelines
...
- **Mapbox GL JS docs**: <https://docs.mapbox.com/mapbox-gl-js/> -- target employer uses Mapbox for map rendering. Understand layers, sources, expressions.
- **GeoJSON specification**: <https://geojson.org/> -- data format for geospatial features (points, polygons, lines).

### Optimization
- **Google OR-Tools**: <https://developers.google.com/optimization> -- vehicle routing, scheduling, constraint programming. Directly relevant to target employer's route optimization.
...
- **PyTorch Geometric**: <https://pyg.org/> -- graph neural networks. Relevant to target employer's graph-based modeling.
...
| Zod (TypeScript) | <https://zod.dev/> | Runtime type validation (target employer uses TypeScript frontend) |
