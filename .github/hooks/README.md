...
AGENTS.md: the agent never edits private practice files.

The guard denies:

- any file-edit/write tool call whose path-shaped arguments reference
  `.challenges/**`, covering candidate-owned work and generated read-only study
  snapshots; a drifted snapshot is regenerated by rerunning `practice-study`
- any shell/terminal tool command that directly references `.challenges/**`,
  ...
  remain neutral because they do not expose private paths. Read generated
  study files with the file-read tool and the exact emitted `STUDY_SOURCE` or
  `STUDY_TEST` path
