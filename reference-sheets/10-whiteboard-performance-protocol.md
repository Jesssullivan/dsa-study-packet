...
1. **Board-dump on arrival (expressive writing).** Before the problem, in the
   ...
   queue; hash-map counting). This externalizes memory so panic cannot erase it;
...
+1 is a transformed interview. Close once with `just rep-finish <topic>
<problem> "<rep line>"`; it logs the line and schedules review together. Note
ONE thing to fix next rep. Example:

```text
just rep-finish arrays two_sum "cold arrays/two_sum C2 L2 A1 R1 P1 h1 trace before optimizing"
```
